<template>
    <section
      class="mx-auto max-w-4xl dark:bg-gray-900 min-h-screen dark:text-gray-100 p-4"
    >
      <!-- Back Button -->
      <button
        @click="$router.go(-1)"
  ...
  const maidProfile = ref({
    first_name: route.query.maidName?.split(" ")[0] || "",
    last_name: route.query.maidName?.split(" ")[1] || "",
    image_url: route.query.maidImage || "",
    phone_number1: "",
    skill: "",
    main_language: "",
    experience: "",
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch job details
      const jobResponse = await backendApi.get(`/job/${jobId}/applications`, {
        headers: {
      ...
      // Only fetch full maid details if we don't have basic info from query params
      if (!route.query.maidId) {
        const applicationResponse = await backendApi.get(
        ...
        if (applicationResponse.data.maid_profile) {
          maidProfile.value = applicationResponse.data.maid_profile;
        }
      } else {
        // Fetch additional maid details if needed
        const maidResponse = await backendApi.get(
          `/maids/${route.query.maidId}`,
          {
            headers: {
              Authorization: `Bearer ${authStore.accessToken}`,
            },
        ...
        maidProfile.value = {
          ...maidProfile.value,
          phone_number1: maidResponse.data.phone_number1 || "",
          skill: maidResponse.data.skill || "",
          main_language: maidResponse.data.main_language || "",
